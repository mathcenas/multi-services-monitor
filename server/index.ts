...
app.get('/api/agent-version', (req, res) => {
  const versions = {
    ...
    'monitor-agent-omv-connections.sh': '1.2.0'
  };
  res.json(versions);
});
  ...
  try {
    ...
    if (serverId) {
      ...
      activeConnections = db.prepare(`
        ...
        LEFT JOIN servers s ON nc.server_id = s.id
        WHERE nc.is_active = 1
        ORDER BY s.name, nc.last_seen DESC
      `).all();
    }

    const now = new Date().toLocaleString();
    const title = serverId && serverInfo
      ? `Connection Inventory - ${serverInfo.name}`
      : 'Connection Inventory - All Servers';
...
<body>
  ...
  <div class="header-info">
    ...
    <strong>Total Active Connections:</strong> ${activeConnections.length}
  </div>

  <table>
    <thead>
      <tr>
        ${!serverId ? '<th>Server</th>' : ''}
        <th>Username</th>
        <th>IP Address</th>
        <th>Hostname</th>
        <th>Protocol</th>
    ...
    <tbody>
      ${activeConnections.map((conn: any) => `
        <tr>
          ${!serverId ? `<td>${conn.server_name || 'Unknown'}</td>` : ''}
          <td>${conn.username || '-'}</td>
          <td>${conn.ip_address || '-'}</td>
          <td>${conn.hostname || '-'}</td>
          <td>${conn.protocol || '-'}</td>
          <td>${conn.share_name || '-'}</td>
          <td>${conn.connected_at ? new Date(conn.connected_at).toLocaleString() : '-'}</td>
          <td>${conn.last_seen ? new Date(conn.last_seen).toLocaleString() : '-'}</td>
        </tr>
      `).join('')}
    </tbody>
  </table>
  ...
  <div class="footer">
    Generated by CENAS Support Monitoring System
  </div>
</body>
</html>`;
